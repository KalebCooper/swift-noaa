...
# Prohibition. SwiftNWSModels builds on every platform the package supports, so it imports Foundation
# only under the `#else` of a `#if canImport(FoundationEssentials)`.
check_models_foundation_import() {
  local name="every import Foundation in Sources/SwiftNWSModels sits under #else"
  local files hits
...
# Prohibition. SwiftNWSModels is usable on any data layer, so it reaches for no networking stack: not
...
# declaration kind), so an import cannot hide behind one; `\b` keeps `SwiftNWSModels` itself out of
# the `SwiftNWS` match.
check_models_import_boundary() {
  local name="no swifty-networking, transport, or SDK import in Sources/SwiftNWSModels"
  local files hits
  files=$(swift_files Sources/SwiftNWSModels)
  if [ -z "$files" ]; then fail "$name (Sources/SwiftNWSModels holds no Swift file; the check has lost its subject)"; return; fi
...
# and its `.json` exclusion for the reason stated there: the provider's own glossary defines seepage
# through seams in a dam abutment and a hurricane's double-eye, and neither one is this repository
# writing about its tests.
check_test_jargon() {
...
write_clean_tree() {
  # $1: directory
  local d="$1"
  mkdir -p "$d/Sources/SwiftNWSModels" "$d/Sources/SwiftNWS" "$d/Sources/SwiftNWSTestSupport/Fixtures" "$d/Tests/SwiftNWSModelsTests" "$d/Tests/SwiftNWSTests" "$d/Scripts"
  ...
  printf '{\n  "glossary": [\n    {\n      "term": "Abutment seepage",\n      "definition": "Water that moves through seams in the abutment \342\200\224 and exits downstream."\n    }\n  ]\n}\n' > "$d/Sources/SwiftNWSTestSupport/Fixtures/Glossary.json"
  ...
  cat > "$d/Sources/SwiftNWSModels/MediaType.swift" <<'EOF'
...
public typealias SwiftNWSModelsVersion = Int
EOF
  cat > "$d/Sources/SwiftNWS/Client.swift" <<'EOF'
import HTTPCore
...
EOF
  ...
  cat > "$d/Tests/SwiftNWSModelsTests/MediaTypeTests.swift" <<'EOF'
import SwiftNWSModels
import Testing
...
EOF
  cat > "$d/Tests/SwiftNWSTests/ClientTests.swift" <<'EOF'
import SwiftNWS
import SwiftNWSModels
import Testing
...
plant_violation() {
  ...
  case "$2" in
    ...
    check_models_foundation_import)
      printf 'import Foundation\n' > "$d/Sources/SwiftNWSModels/Unguarded.swift" ;;
    check_models_import_boundary)
...
plant_fifth_violation() {
  ...
  case "$2" in
    ...
    check_suite_time_limit)
      printf 'import Testing\n\n@Test func aTestAtFileScopeRuns() {\n  #expect(true)\n}\n' > "$d/Tests/SwiftNWSTests/FileScopeTests.swift" ;;
    *) return 1 ;;
  esac
}
