...
public struct WeatherRequest<Response>: Hashable, Sendable {
  ...
  public enum Resolution: Hashable, Sendable {
    ...
    /// Retrieve the latest product of one kind issued for one location.
    ///
    /// Only requests returning ``TextProduct`` carry this resolution. An empty code or location
    /// identifier, or one that produces an invalid encoded path, is a failure before any request;
    /// the code is checked first. The service selects the product, so this is one request rather
    /// than a list followed by a detail lookup.
    case latestProduct(location: String, type: ProductCode)
    ...
    /// Retrieve one text product by identifier.
    ///
    /// Only requests returning ``TextProduct`` carry this resolution. An empty identifier, or one
    /// that produces an invalid encoded path, is a failure before any request.
    ...
    /// Retrieve the text products of one kind, optionally narrowed to one location.
    ///
    /// Only requests returning ``TextProducts`` carry this resolution. A nil location selects
    /// `/products/types/{typeId}`, and a location selects
    /// `/products/types/{typeId}/locations/{locationId}`. An empty code or location identifier, or
    /// one that produces an invalid encoded path, is a failure before any request; the code is
    /// checked first. The routes declare no cursor, so nothing is paged.
    case productsOfType(location: String?, type: ProductCode)
...
extension WeatherRequest where Response == TextProduct {
  /// Describes the latest product of one kind issued for one location,
  /// `/products/types/{typeId}/locations/{locationId}/latest`.
  ///
  /// The service selects the product, so executing the request sends one request rather than
  /// listing products and fetching one. An unusable code or location identifier is reported when
  /// the request is executed, not when it is created.
  ///
  /// ```swift
  ...
  /// - Returns: The request for that product.
  public static func latestProduct(at location: String, ofType type: ProductCode) -> Self {
    Self(resolution: .latestProduct(location: location, type: type))
  }

  /// Describes the latest product of one kind issued for one location using a consumer-defined
  /// product code enum.
  /// - Parameters:
  ...
  /// Describes one text product by identifier, `/products/{productId}`.
  ///
  /// The request returns the product with its text. An unusable identifier is reported when the
  /// request is executed, not when it is created.
  ///
  /// ```swift
  /// let request = WeatherRequest.product(identifier: "a6addd61-6620-4718-9d53-effd7d8c2560")
  /// ```
  ///
  ...
  public static func product(identifier: String) -> Self {
    Self(resolution: .product(identifier: identifier))
  }
}

extension WeatherRequest where Response == TextProducts {
  /// Describes the text products matching a validated query, `/products`.
  ///
  /// The request sends ``Endpoint/products(matching:)`` and returns its body unchanged. The route
  /// declares no cursor, so nothing continues the list, and the entries carry no product text.
  ///
  /// ```swift
  /// let query = try ProductQuery(limit: 2, locations: ["EWX"], types: [.areaForecastDiscussion])
  /// let request = WeatherRequest.products(matching: query)
  /// ```
  ///
  /// - Parameter query: The validated filters, window, and page size.
  /// - Returns: The request for that query.
  public static func products(matching query: ProductQuery) -> Self {
    Self(endpoint: .products(matching: query))
  }

  /// Describes the text products of one kind issued for one location,
  /// `/products/types/{typeId}/locations/{locationId}`.
  ///
  /// An unusable code or location identifier is reported when the request is executed, not when it
  /// is created. The entries carry no product text.
  ///
  /// ```swift
  /// let request = WeatherRequest.products(at: "EWX", ofType: .areaForecastDiscussion)
  /// ```
  ///
  /// - Parameters:
  ///   - location: The product location's identifier, such as `EWX`.
  ///   - type: The product's code, such as ``ProductCode/areaForecastDiscussion``.
  /// - Returns: The request for that list.
  public static func products(at location: String, ofType type: ProductCode) -> Self {
    Self(resolution: .productsOfType(location: location, type: type))
  }

  /// Describes the text products of one kind issued for one location using a consumer-defined
  /// product code enum.
  /// - Parameters:
  ///   - location: The product location's identifier.
  ///   - type: A String-backed product code.
  ...
  /// Describes the text products of one kind, `/products/types/{typeId}`.
  ///
  /// An unusable code is reported when the request is executed, not when it is created. The
  /// entries carry no product text.
  ///
  /// ```swift
  /// let request = WeatherRequest.products(ofType: .areaForecastDiscussion)
  /// ```
  ///
  /// - Parameter type: The product's code, such as ``ProductCode/areaForecastDiscussion``.
  /// - Returns: The request for that list.
  public static func products(ofType type: ProductCode) -> Self {
    Self(resolution: .productsOfType(location: nil, type: type))
  }

  /// Describes the text products of one kind using a consumer-defined product code enum.
  /// - Parameter type: A String-backed product code.
  /// - Returns: The request for that list.
